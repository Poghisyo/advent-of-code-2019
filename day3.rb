...
def plot_path(path)
  ...
  path.each do |coord|
    p_coord = parse_input(coord)
    new_positions = []
    current_x = plotted.last[0]
    current_y = plotted.last[1]

    case p_coord[0]
    when "R"
      new_x = p_coord[1].to_i + current_x
      (current_x..new_x).to_a.each{ |x| new_positions << [x, current_y] }

      plotted += new_positions[1..-1]
    when "L"
      new_x = current_x - p_coord[1].to_i
      (new_x..current_x).to_a.reverse.each{ |x| new_positions << [x, current_y] }

      plotted += new_positions[1..-1]

    when "U"
      new_y = p_coord[1].to_i + current_y
      (current_y..new_y).to_a.each{ |y| new_positions << [current_x, y] }

      plotted += new_positions[1..-1]

    when "D"
      new_y = current_y - p_coord[1].to_i
      (new_y..current_y).to_a.reverse.each{ |y| new_positions << [current_x, y] }
...
def find_in_route(path, intersection)
  """returns array of coordinates [x,y]"""
  plotted = [[0, 0]]
  distance = 0
  ...
  path.each do |coord|
    ...
    when "R"
      new_x = p_coord[1].to_i + current_x
      (current_x..new_x).to_a.each_with_index do |x, i|
        new_positions << [x, current_y]
        if intersection ==  [x, current_y]
          return distance + i
        end
      end
      distance += p_coord[1].to_i
      plotted += new_positions[1..-1]
    when "L"
      new_x = current_x - p_coord[1].to_i
      (new_x..current_x).to_a.reverse.each_with_index do |x, i|
        new_positions << [x, current_y]
        if intersection ==  [x, current_y]
          return distance + i
        end
    ...
    when "U"
      new_y = p_coord[1].to_i + current_y
      (current_y..new_y).to_a.each_with_index do |y, i|
        new_positions << [current_x, y]
        if intersection ==  [current_x, y]
          return distance + i
        end
      end
      distance += p_coord[1].to_i
    ...
    when "D"
      new_y = current_y - p_coord[1].to_i
      (new_y..current_y).to_a.reverse.each_with_index do |y, i|
        new_positions << [current_x, y]
        if intersection ==  [current_x, y]
          return distance + i
        end
...
def find_closest(intersections)
  closest = intersections[0][0].abs + intersections[0][1].abs
  intersections.each do |intersection|
    if intersection[0].abs + intersection[1].abs < closest
      closest = intersection[0].abs + intersection[1].abs
    end
  end
  return closest
end

def measure_distance(path_1, path_2, intersections)
  """
  takes paths and intersections and finds first instance of each intersection in each path
  measures distance of each instance and add them to get total distance
  and measure if this distance is the shortest path for each intersection
  retrun shortest distance
  """
  # distance_path_1 = find_in_route(path_1, intersection)
  # distance_path_2 = find_in_route(path_2, intersection)
  distances = []
  ...
  intersections.each do |intersection|
    distance_path_1 = find_in_route(path_1, intersection)
    distance_path_2 = find_in_route(path_2, intersection)
    distances << distance_path_1 + distance_path_2
    # p distance_path_1
    # p distance_path_2
  end
  distances.sort
...
def method_name(path_1, path_2)
  wire1 = plot_path(path_1)
  wire2 = plot_path(path_2)
  intersections = (wire1 & wire2)
  # p wire2[-1]
  # p intersections.length

  p measure_distance(path_1, path_2, intersections[1..-1])[0]
  # closest = find_closest(intersections[1..-1])
  # p closest
end

method_name(PATH1, PATH2)


# p find_in_route(["R8","U5","L5","D3"], [8, 2])
# p find_in_route(["R8","U5","L5","D3"], [5, 5])
# p find_in_route(["R8","U5","L5","D3"], [3, 4])

# p plot_points(["R8","U5","L5","D3"])
# p find_intersections([[0,0], [4, 0], [4, 3], [5, 3]], [[0,0], [0, 3], [4, 3], [4, 6]])
# [[0,0], [4, 0], [4, 3], [5, 3]]
# [[0,0], [0, 3], [4, 3], [4, 6]]


# tests
# method_name(["R98","U47","R26","D63","R33","U87","L62","D20","R33","U53","R51"], ["U98","R91","D20","R16","D67","R40","U7","R15","U6","R7"])
